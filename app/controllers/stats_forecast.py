...
from statsforecast.models import AutoARIMA, HistoricAverage, Naive, RandomWalkWithDrift, GARCH, AutoETS, AutoCES, \
    AutoTheta

from app.models.nixtla_models import ForecastRequest
...
def naive(request: ForecastRequest) -> JSONResponse:
...
@stats_forecast_router.post('/auto-ets')
def garch(request: List[Candle]) -> JSONResponse:
    forecast_df = stats_predict(request, AutoETS())
    return to_result(forecast_df.iloc[0]['AutoETS'], request[-1].time_utc)

@stats_forecast_router.post('/auto-ces')
def garch(request: List[Candle]) -> JSONResponse:
    forecast_df = stats_predict(request, AutoCES())
    return to_result(forecast_df.iloc[0]['CES'], request[-1].time_utc)

@stats_forecast_router.post('/auto-theta')
def garch(request: List[Candle]) -> JSONResponse:
    forecast_df = stats_predict(request, AutoTheta())
    return to_result(forecast_df.iloc[0]['AutoTheta'], request[-1].time_utc)
